...
export const publicRequestFetch = async (domain: string): Promise<{redirect?: false | string, id: string, tags: string, request: CustomFetchBackendInterface}> => {
  const getDomainSubdomain = domain.indexOf('.') ? {subdomain: domain.split('.')[0].replace('www', '')} : {domain};
  const {data: {apiKey, id}} = await customFetchBackend().get(
  ...
  return {redirect, id, tags: getDomainSubdomain?.domain || getDomainSubdomain?.subdomain || '', request: customFetchBackend(undefined, {apikey: apiKey, serverkey: process.env.BACKEND_TOKEN_PROTECTOR, tags: [getDomainSubdomain?.domain || getDomainSubdomain?.subdomain || '']})};
}
